...
public class CurrentReceiveData {
	...
	static List<JSONObject> faultBaseJsonLists = new ArrayList<JSONObject>(); // 实时异常存数据库数据缓存(包括预 警、故障、报警)
	...
	static String systemName = "电气火灾---";
	static String orgIp;
	static NodeManagerDao nodeDao = new NodeManagerDao();
	static Log currentReceive_log = LogFactory.getLog(CurrentReceiveData.class);
	public static String[] currentTableHeader = new String[] { "节点编号", "设备位置", "电流1", "电流2", "电流3", "电流4", "温度1", "温度2", "温度3", "温度4", "更新时间" };
	...
	static class recieveData implements Runnable {
		...
		public void run() {
			...
			try {
				serverSocket = new UdpServerSocket(CacheDataBase.basicConfig.getString("host"), CacheDataBase.basicConfig.getInt("currentport"));
				while (true) {
				...
				synchronized (refreshParamLock) {
					...
					for (int i = 0; i < nodeDataArray.size(); i++) {
						...
						if (!CacheDataBase.subSystemNodeData.containsKey(nodeId)) {
							currentReceive_log.error("数据库中无此节点:" + nodeId);
							break;
						}
						List<String> list = new ArrayList<String>();
						list.add(nodeId);
						list.add(CacheDataBase.subSystemNodeData.get(nodeId).getString("nodeLocation"));
						list.add(jsonObject.getString("current1"));
						list.add(jsonObject.getString("current2"));
						list.add(jsonObject.getString("current3"));
						list.add(jsonObject.getString("current4"));
						list.add(jsonObject.getString("temperature1"));
						list.add(jsonObject.getString("temperature2"));
						list.add(jsonObject.getString("temperature3"));
						list.add(jsonObject.getString("temperature4"));
						list.add(jsonObject.getString("updatetime").replace("T", " "));
						list.add(jsonObject.getString("faulttype"));
						jsonObject.remove("faulttype");
						if (CacheDataBase.currentMainValues.containsKey(buildId)) {
							if (CacheDataBase.currentMainValues.get(buildId).containsKey(canNo)) {
								CacheDataBase.currentMainValues.get(buildId).get(canNo).put(nodeId, list);
							} else {
								JSONObject jObject = new JSONObject();
								jObject.put(nodeId, list);
								CacheDataBase.currentMainValues.get(buildId).put(canNo, jObject);
							}
						} else {
							JSONObject jObject = new JSONObject();
							jObject.put(nodeId, list);
							...
							CacheDataBase.currentMainValues.put(buildId, map);
						}
					}
				}
			}
			...
			if (!dataJsonObject.getString("faulttype").equals("0")) {
				...
				synchronized (saveBaselock) {
					...
					if (saveBaseDataThread.getState().toString().equals("TERMINATED")) {
						saveBaseDataThread = new Thread(new saveFaultData());//异常信息存入数据库线程开启
						saveBaseDataThread.start();
					}
					saveBaselock.notify();
				}
