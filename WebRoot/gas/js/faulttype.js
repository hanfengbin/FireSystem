$(document).ready(function(){
    function GetQueryString(name)
    {
     ...
     $type=$type.split(",")[1];
     var $systemType="";
     if($id.substr($id.length-1,1)=="a"){
    	 $systemType="gas1"; 
     }else if($id.substr($id.length-1,1)=="b"){
    	 $systemType="gas2"; 
     }else {
    	 $systemType="gas"
     }
     console.log($type+$id+$systemType);
	$.ajax({
                        type: "post",
                        url: "../servlet/DispatchServlet",
                        data:{'controller':'FaultTypeController','system':$systemType,'faultType':$type},
                        dataType: "json",
