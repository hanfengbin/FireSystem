...
public class FireDoorReceiveData {
	...
	static String systemName = "防火门---";
	static String orgIp;
	static NodeManagerDao nodeDao = new NodeManagerDao();
	public static String[] fireDoorTableHeader = new String[] { "设备编号", "设备位置", "电动闭门器1", "电动闭门器2", "门磁开关1", "门磁开关2", "门磁释放器1", "门磁释放器2", "门状态", "温度", "更新时间" };
	...
	static synchronized boolean unpackPackage(JSONObject currentPackJsonObject, Integer len) {
		/*
		 * if
		 * (!currentPackJsonObject.containsKey("len")&&currentPackJsonObject.getInt
		 * ("len") != len) { //System.out.println(systemName + "数据包格式不对");
		 * return false; } else {
		 */
		...
		if (state.equals("currentstate")) {
			...
			try {
				if (state.equals("add")) {
					nodeDao.addNode(currentPackJsonObject);
				} else if (state.equals("modify")) {
					nodeDao.updateNode(currentPackJsonObject);
				} else if (state.equals("delete")) {
					nodeDao.deleteNode(currentPackJsonObject);
				}
			} catch (Exception e) {
				// TODO: handle exception
				e.printStackTrace();
				try {
					Client.client.send(orgIp, 3344, "error".getBytes());
			...
			try {
				Client.client.send(orgIp, 3344, "success".getBytes());
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	static class recieveData implements Runnable {
		...
		public void run() {
			...
			try {
				serverSocket = new UdpServerSocket(CacheDataBase.basicConfig.getString("host"), CacheDataBase.basicConfig.getInt("doorport"));
				while (true) {
			...
			while (true) {
				synchronized (refreshParamLock) {
					...
					for (int i = 0; i < nodeDataArray.size(); i++) {
						...
						list.add(jsonObject.getString("Close1"));
						list.add(jsonObject.getString("Close2"));
						list.add(jsonObject.getString("Magnetic1"));
						list.add(jsonObject.getString("Magnetic2"));
						list.add(jsonObject.getString("Release1"));
						list.add(jsonObject.getString("Release2"));
						list.add(jsonObject.getString("Status"));
						list.add(jsonObject.getString("Temper"));
						list.add(jsonObject.getString("Updatetime").replace("T", " "));
						if (CacheDataBase.fireDoorMainValues.containsKey(buildId)) {
							if (CacheDataBase.fireDoorMainValues.get(buildId).containsKey(canNo)) {
								CacheDataBase.fireDoorMainValues.get(buildId).get(canNo).put(nodeId, list);
							} else {
								JSONObject jObject = new JSONObject();
								jObject.put(nodeId, list);
								CacheDataBase.fireDoorMainValues.get(buildId).put(canNo, jObject);
							}
						} else {
							JSONObject jObject = new JSONObject();
							jObject.put(nodeId, list);
							...
							CacheDataBase.fireDoorMainValues.put(buildId, map);
						}
					}
	...
	static void judgeFaultParam(JSONObject jsonObject) {
		...
		// //System.out.println("judge：" + nodeArray);
		for (int i = 0; i < nodeArray.size(); i++) {
			JSONObject dataJsonObject = nodeArray.getJSONObject(i);
			if (dataJsonObject.getString("Status").equals("0")) {
				String faultType = "0";
				...
				faultjsonObject.put("faultLocation", CacheDataBase.subSystemNodeData.get(deviceID).getString("nodesLocation"));
		...
		for (int i = 0; i < mainArray.size(); i++) {
			...
			if (!hostJsonObject.containsKey("faultType")) {
				break;
			}
