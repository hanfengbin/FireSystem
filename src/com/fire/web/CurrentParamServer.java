...
@ServerEndpoint("/currentParam.do")
public class CurrentParamServer {
	...
	public void onMessage(String message, final Session session) throws IOException {
		...
		if (jsonObject.containsKey("buildId")) {
			...
			if (CacheDataBase.currentMainValues.containsKey(buildId)) {
				sendData.put("tableHeader", CurrentReceiveData.currentTableHeader);
				sendData.put("tableContent", CacheDataBase.currentMainValues.get(buildId).get(canNo));
